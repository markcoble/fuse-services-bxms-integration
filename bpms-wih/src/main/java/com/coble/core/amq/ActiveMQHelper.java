...
import com.coble.core.utils.PropertyLoader;
...
public final class ActiveMQHelper {
	...
	private final String brokerName = "amq";
	...
	public ActiveMQHelper() {
		...
		try {
			...
			ConnectionFactory fac = new ActiveMQConnectionFactory(
					PropertyLoader.getEnvironmentProperty("activemq.brokerurl"));
			connection = fac.createConnection(
					PropertyLoader.getEnvironmentProperty("activemq.username"),
	...
	public ActiveMQHelper(final String brokerUrl) {
		try {
			...
			ConnectionFactory fac = new ActiveMQConnectionFactory(
					PropertyLoader.getEnvironmentProperty(brokerUrl));
			connection = fac.createConnection(
					PropertyLoader.getEnvironmentProperty("activemq.username"),
					PropertyLoader.getEnvironmentProperty("activemq.password"));
			connection.start();
	...
	public List<Message> consumeMessages(final String queue,
			final String messageSelector, int expectedMessages, int timeout)
			throws Exception {
	 ...
	 */
	public Message consumeMessage(final String queue, int timeout,
		...
		try {
			...
			consumer = messageSelector != null ? session.createConsumer(
					destination, messageSelector) : session
					.createConsumer(destination);
			return consumer.receive(timeout);
		} catch (JMSException e) {
			logger.error("Failed to create session for given queue {}", queue,
					e);
			return null;
		} finally {
			if (session != null) {
				try {
					...
					logger.warn(
							"Failed to close the session for given queue {}",
							queue, e);
				}
			}
		}
	 ...
	 * @return true if message got successfully published.
	 */
	public boolean publishMessage(final String queue, Serializable message,
			Map<String, String> headers, String replyTo) {
		Session session = null;
	 ...
	 */
	/*
	 * public boolean purgeQueue(final String queue) { return
	 * purgeQueueViaJkia(queue); }
	 */
	/**
	 * Purge a queue. Useful before running a test.
	 * 
	 ...
	 */
	/*
	 * private boolean purgeQueueViaJkia(final String queue) { String user =
	 * PropertyLoader.getEnvironmentProperty("activemq.username"); String pwd =
	 * PropertyLoader.getEnvironmentProperty("activemq.password");
	 * 
	 * //Jolokia Url For Mbean Operation StringBuilder jkUrlfMbOp = new
	 * StringBuilder(); jkUrlfMbOp.append(PropertyLoader.getEnvironmentProperty(
	 * "jolokia.activemq.brokerurl"));
	 * jkUrlfMbOp.append("/exec/org.apache.activemq:type=Broker,brokerName=");
	 * jkUrlfMbOp
	 * .append(PropertyLoader.getEnvironmentProperty("jolokia.activemq.brokername"
	 * )); jkUrlfMbOp.append(",destinationType=Queue,destinationName=");
	 * jkUrlfMbOp.append(queue); jkUrlfMbOp.append("/purge");
	 * 
	 * int status= with() .auth().basic(user, pwd) .get(jkUrlfMbOp.toString())
	 * .statusCode();
	 * 
	 * return status==200; }
	 */
	...
	private boolean purgeQueueViaJMX(final String queue) {
		...
		try {
			...
			QueueViewMBean queueProxy = (QueueViewMBean) MBeanServerInvocationHandler
					.newProxyInstance(conn, queueObjectName,
			...
			logger.error("Failed to create JMX connection to purge queue",
					queue, e);
			return false;
		} finally {
			if (session != null) {
	...
	private void setHeaders(Message msg, Map<String, String> headers) {
		...
		for (Map.Entry<String, String> entry : headers.entrySet()) {
			try {
				...
				logger.error("Failed to set message headers {}",
						headers.toString());
