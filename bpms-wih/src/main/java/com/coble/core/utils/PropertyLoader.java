...
public class PropertyLoader {
		...
		if (!properties.containsKey(propertyName)) {
			try {
				if (logger.isDebugEnabled())
					logger.debug("/properties/" + environment + ".properties");
	...
	/**
	 * This method can be used to load a property from the property file should
	 * be used for loading properties in the Misc Properties section
	 *
	 * @param propertyName
	 * @return the value of the property
	 */
	...
	/**
	 * determines the environment that is specified when running the tests this
	 * is taken from the environment property in the pom.xml you can specify the
	 * environment when running using maven with the following argument:
	 * <p/>
	 * -Denvironment=local
	 * <p/>
	 * for example to run tests against dev12 use: -Denvironment=dev12
	 *
	...
	public static String getEnvironment() {
		...
		if (null == environment || environment.isEmpty()) {
			...
			System.out.println("Using default environment of [" + environment + "]");
